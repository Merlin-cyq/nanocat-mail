...
// ---------- 读取登录用户信息 ----------
(function initUser() {
  var accountKey = "nanocat-account";
  var tokenKey = "nanocat-token";
  var token = localStorage.getItem(tokenKey);
  var account = null;
  try { account = JSON.parse(localStorage.getItem(accountKey)); } catch(e) {}
  ...
  if (userInitial) userInitial.textContent = (account.displayName || account.email || "N").charAt(0).toUpperCase();
  if (userName) userName.textContent = account.displayName || account.email.split("@")[0] || "用户";
  if (userEmail) userEmail.textContent = account.email || "@nanocat.xin";
})();
...
const starterMail = [
  ...
  },
  {
    id: "m-1002",
    folder: "inbox",
    from: getUserEmail() + " <" + getUserEmail() + ">",
...
function getUserEmail() {
  try {
    var account = JSON.parse(localStorage.getItem("nanocat-account"));
    return account ? account.email : "admin@nanocat.xin";
  } catch(e) {
    return "admin@nanocat.xin";
  }
}
